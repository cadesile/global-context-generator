...
# Lists all markdown files as plain links grouped by directory — no AI calls.
list_context_files() {
    local EXCLUDE_PATTERNS='node_modules|vendor|.git|dist|build|__pycache__'
    local prev_dir=""
    find . -name "*.md" -not -path "*/.*" | grep -vE "$EXCLUDE_PATTERNS" | sort | while read -r f; do
        [ -z "$f" ] && continue
        local rel_path=${f#./}
        local dir
...
# ── Stack-specific schema scanners ───────────────────────────────────────────

# React Native / Expo (SQLite via expo-sqlite) — schema.ts or .sql files
_schema_sqlite() {
    local schema_files
    schema_files=$(find . \( -name "schema.ts" -o -name "schema.sql" -o -name "*.sql" \) \
        -not -path "*/node_modules/*" -not -path "*/.git/*" 2>/dev/null | sort | head -5)
    [ -z "$schema_files" ] && echo "_No schema file found._" && return
    echo "$schema_files" | while read -r f; do
        [ -z "$f" ] && continue
        echo "**\`${f#./}\`**"
        echo '```sql'
        awk '
            /CREATE TABLE/{ in_block=1; buf="" }
            in_block{ buf = buf $0 "\n" }
            /;$/ && in_block{ printf "%s\n", buf; in_block=0; buf="" }
        ' "$f" 2>/dev/null
        echo '```'
...
# Symfony — Doctrine migrations (last 10) + entity field listing
_schema_doctrine() {
    local mdir="${APP_DIR}/migrations"
    if [ -d "$mdir" ]; then
        echo "**Migrations (latest 10):**"
        echo ""
        find "$mdir" -name "*.php" -type f 2>/dev/null | sort | tail -10 | while read -r f; do
            local name; name=$(basename "$f" .php)
            echo "- \`${name}\`"
            grep -oE "addSql\('[^']+'" "$f" 2>/dev/null | head -3 | sed "s/addSql('//;s/'$//" | sed 's/^/  → /'
        done
        echo ""
    fi
    if [ -n "$MODELS_DIR" ] && [ -d "$MODELS_DIR" ]; then
        echo "**Entity column/relation map:**"
        echo ""
        find "$MODELS_DIR" -name "*.php" -type f 2>/dev/null | sort | while read -r f; do
            local name; name=$(basename "$f" .php)
            local has_entity
            has_entity=$(grep -c "#\[ORM\\\\Entity\]\|@ORM\\\\Entity" "$f" 2>/dev/null || echo 0)
            [ "$has_entity" -eq 0 ] && continue
            echo "#### \`${name}\`"
            echo '```php'
            grep -E '#\[ORM\\\\(Column|Id|ManyToOne|OneToMany|ManyToMany|OneToOne|JoinColumn)|@ORM\\\\(Column|Id|ManyTo|OneMany|JoinColumn)|\$\w+' \
                "$f" 2>/dev/null | grep -v '^\s*//' | head -25
            echo '```'
            echo ""
        done
    fi
...
# Laravel — database/migrations (last 10) + Eloquent model relationships/fillable
_schema_laravel() {
    ...
    if [ -d "$mdir" ]; then
        ...
        find "$mdir" -name "*.php" -type f 2>/dev/null | sort | tail -10 | while read -r f; do
            local name; name=$(basename "$f" .php)
            echo "- \`${name}\`"
            grep -E "->string|->integer|->boolean|->foreignId|->timestamps|->text|->decimal" \
                "$f" 2>/dev/null | head -4 | sed 's/^\s*/  /'
        done
        echo ""
    fi
    local mdir_models="${APP_DIR}/app/Models"
    [ ! -d "$mdir_models" ] && mdir_models="app/Models"
    if [ -d "$mdir_models" ]; then
        echo "**Eloquent models:**"
        echo ""
        find "$mdir_models" -name "*.php" -type f 2>/dev/null | sort | while read -r f; do
            local name; name=$(basename "$f" .php)
            echo "#### \`${name}\`"
            echo '```php'
            grep -E "protected \\\$fillable|protected \\\$casts|protected \\\$table|public function (belongsTo|hasMany|hasOne|belongsToMany|morphTo|morphMany)" \
...
_schema_django() {
    find . -name "models.py" -not -path "*/.git/*" -not -path "*/node_modules/*" \
        2>/dev/null | sort | while read -r f; do
        local rel="${f#./}"
        echo "#### \`${rel}\`"
        echo '```python'
        grep -E "^class [A-Z]|^\s+\w+ = models\.|^\s+class Meta|^\s+ordering|^\s+db_table" \
...
_schema_rails() {
    if [ -f "db/schema.rb" ]; then
        echo "**\`db/schema.rb\`**"
        echo '```ruby'
        grep -E "create_table|t\.(string|integer|boolean|text|datetime|references|belongs_to|index)|end" \
            db/schema.rb 2>/dev/null | head -80
        echo '```'
    else
        echo "_No \`db/schema.rb\` found._"
    fi
}

# Go — struct definitions from models dir
_schema_go() {
    [ -z "$MODELS_DIR" ] || [ ! -d "$MODELS_DIR" ] && return
    find "$MODELS_DIR" -name "*.go" -type f 2>/dev/null | sort | while read -r f; do
        local rel="${f#./}"
        echo "#### \`${rel}\`"
        echo '```go'
        awk '/^type .* struct \{/{p=1} p{print} /^\}$/ && p{p=0; print ""}' "$f" 2>/dev/null | head -60
        echo '```'
        echo ""
    done
}

# ── Stack-specific entity definition scanners ────────────────────────────────

# TypeScript (React Native / Node) — full exported interfaces, types, enums from src/types/
_entities_typescript() {
    local types_dir="${SOURCE_DIR}/types"
    [ -d "$types_dir" ] || types_dir="src/types"
    [ -d "$types_dir" ] || return

    find "$types_dir" -name "*.ts" -not -name "*.test.ts" -type f 2>/dev/null | sort | while read -r f; do
        local rel="${f#./}"
        local has_exports
        has_exports=$(grep -c "^export \(interface\|type\|enum\)" "$f" 2>/dev/null || echo 0)
        [ "$has_exports" -eq 0 ] && continue
        echo "#### \`${rel}\`"
        echo '```typescript'
        awk '
            /^export (interface|type|enum) /{ p=1; depth=0; buf="" }
            p {
                buf = buf $0 "\n"
                for(i=1; i<=length($0); i++) {
                    c = substr($0,i,1)
                    if(c=="{") depth++
                    if(c=="}") { depth--; if(depth==0){ printf "%s\n", buf; p=0; buf="" } }
                }
                if(p && depth==0 && index($0,";")>0 && index($0,"{")==0){ printf "%s\n", buf; p=0; buf="" }
            }
        ' "$f" 2>/dev/null | head -120
        echo '```'
        echo ""
    done
}

# PHP (Doctrine entities) — class + property declarations
_entities_doctrine() {
    [ -z "$MODELS_DIR" ] || [ ! -d "$MODELS_DIR" ] && return
    find "$MODELS_DIR" -name "*.php" -type f 2>/dev/null | sort | while read -r f; do
        local name; name=$(basename "$f" .php)
        echo "#### \`${name}\`"
        echo '```php'
        grep -E '^\s*(#\[|@ORM|private|protected|public)\s+\$|\s+@var\s' "$f" 2>/dev/null | \
            grep -v '^\s*//' | head -25
        echo '```'
        echo ""
    done
...
# PHP (Eloquent) — same as schema but focused on casts + relationships
_entities_eloquent() {
    local mdir="${APP_DIR}/app/Models"
    [ ! -d "$mdir" ] && mdir="app/Models"
    [ ! -d "$mdir" ] && return
    find "$mdir" -name "*.php" -type f 2>/dev/null | sort | while read -r f; do
        local name; name=$(basename "$f" .php)
        echo "#### \`${name}\`"
        echo '```php'
        grep -E "protected \\\$fillable|protected \\\$casts|protected \\\$hidden|public function " \
            "$f" 2>/dev/null | head -20
        echo '```'
        echo ""
    done
...
# Python (Django) — class-level field definitions
_entities_django() {
    find . -name "models.py" -not -path "*/.git/*" 2>/dev/null | sort | while read -r f; do
        local rel="${f#./}"
        echo "#### \`${rel}\`"
        echo '```python'
        awk '/^class [A-Z]/{p=1} p{print} /^class [A-Z]/ && NR>1{p=1}' "$f" 2>/dev/null | head -60
        echo '```'
        echo ""
    done
}

# Ruby (Rails) — associations, validations, scopes
_entities_rails() {
    [ ! -d "app/models" ] && return
    find app/models -name "*.rb" -type f 2>/dev/null | sort | while read -r f; do
        local name; name=$(basename "$f" .rb)
        echo "#### \`${name}\`"
        echo '```ruby'
        grep -E "^\s*(belongs_to|has_many|has_one|has_and_belongs_to_many|validates|scope|enum|attribute)" \
            "$f" 2>/dev/null | head -20
        echo '```'
        echo ""
    done
...
# Go — type and interface definitions
_entities_go() {
    _schema_go  # struct defs serve as both schema and entity definitions in Go
}

# ── Stack-specific state layer (Zustand / Redux / Pinia etc.) ─────────────────

# TypeScript (Zustand) — interface declarations from src/stores/
_state_zustand() {
    local stores_dir="${SOURCE_DIR}/stores"
    [ -d "$stores_dir" ] || stores_dir="src/stores"
    [ -d "$stores_dir" ] || return
    find "$stores_dir" -name "*.ts" -not -name "*.test.ts" -type f 2>/dev/null | sort | while read -r f; do
        local name; name=$(basename "$f" .ts)
        local has_interface
        has_interface=$(grep -c "^interface \|^export interface " "$f" 2>/dev/null || echo 0)
        [ "$has_interface" -eq 0 ] && continue
        echo "#### \`${name}\`"
        echo '```typescript'
        awk '
            /^(export )?interface /{ p=1; depth=0; buf="" }
            p {
                buf = buf $0 "\n"
                for(i=1; i<=length($0); i++) {
                    c = substr($0,i,1)
...
# Compute human-readable labels for section headings (set before write phase)
DB_SCHEMA_LABEL="Database Schema"
ENTITY_LABEL="Entity Definitions"
STATE_LABEL=""  # empty = section omitted for non-Node stacks

if [ "$STACK_NODE" = true ]; then
    DB_SCHEMA_LABEL="Database Schema (SQLite)"
    ENTITY_LABEL="TypeScript Entity Definitions"
    STATE_LABEL="Store Shapes (State)"
elif [ "$STACK_SYMFONY" = true ]; then
    DB_SCHEMA_LABEL="Database Schema (Doctrine / ${DB_HINTS:-SQL})"
    ENTITY_LABEL="Doctrine Entity Definitions"
elif [ "$STACK_LARAVEL" = true ]; then
    DB_SCHEMA_LABEL="Database Schema (Eloquent / ${DB_HINTS:-SQL})"
    ENTITY_LABEL="Eloquent Model Definitions"
elif [ "$STACK_DJANGO" = true ]; then
    DB_SCHEMA_LABEL="Database Schema (Django ORM / ${DB_HINTS:-SQL})"
    ENTITY_LABEL="Django Model Definitions"
elif [ "$STACK_RAILS" = true ]; then
    DB_SCHEMA_LABEL="Database Schema (ActiveRecord / ${DB_HINTS:-SQL})"
    ENTITY_LABEL="ActiveRecord Model Definitions"
elif [ "$STACK_GO" = true ]; then
    DB_SCHEMA_LABEL="Database Schema (Go structs)"
    ENTITY_LABEL="Go Type Definitions"
fi

scan_db_schema() {
    if   [ "$STACK_NODE"    = true ]; then _schema_sqlite
    elif [ "$STACK_SYMFONY" = true ]; then _schema_doctrine
    elif [ "$STACK_LARAVEL" = true ]; then _schema_laravel
    elif [ "$STACK_DJANGO"  = true ]; then _schema_django
    elif [ "$STACK_RAILS"   = true ]; then _schema_rails
    elif [ "$STACK_GO"      = true ]; then _schema_go
    else echo "_No schema scanner available for detected stack (${PRIMARY_FRAMEWORK})._"
    fi
}

scan_entity_definitions() {
    if   [ "$STACK_NODE"    = true ]; then _entities_typescript
    elif [ "$STACK_SYMFONY" = true ]; then _entities_doctrine
    elif [ "$STACK_LARAVEL" = true ]; then _entities_eloquent
    elif [ "$STACK_DJANGO"  = true ]; then _entities_django
    elif [ "$STACK_RAILS"   = true ]; then _entities_rails
    elif [ "$STACK_GO"      = true ]; then _entities_go
    else echo "_No entity scanner available for detected stack (${PRIMARY_FRAMEWORK})._"
    fi
}

scan_state_layer() {
    [ -z "$STATE_LABEL" ] && return
    if [ "$STACK_NODE" = true ]; then _state_zustand; fi
}
...
echo "---"
echo ""
echo "## ${DB_SCHEMA_LABEL}"
echo ""

info "Extracting database schema..."
scan_db_schema
...
info "Extracting entity definitions..."
scan_entity_definitions
...
if [ -n "$STATE_LABEL" ]; then
    ...
    echo "## ${STATE_LABEL}"
    echo ""
    info "Extracting store interfaces..."
    scan_state_layer
fi
