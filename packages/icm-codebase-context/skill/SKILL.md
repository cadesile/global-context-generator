...
| User says something like "warm", "warm the context", or `/icm-context warm` | Run every stage under `.context/stages/` whose `output/` is empty or missing, in order (`01_overview` → `06_synthesis`), respecting each stage's Checkpoints and Audits — same action as first-run warming above, just explicitly invoked instead of inferred. Leave stages that already have output alone (that's the "explicitly asks to regenerate/refresh" row below) |
